...
input double TrailingStop_Points = 50;  // 移动止损点数
input double TrailingStep_Points = 10;  // 移动止损步长
input double MaxStopLoss_Points = 500;  // 最大止损点数
...
bool CheckBuySignal()
{
    ...
    //--- MACD条件
    bool difCrossUp = (macdLineBuffer[0] > signalLineBuffer[0]) && (macdLineBuffer[1] <= signalLineBuffer[1]);
    bool macdHistPositive = (macdLineBuffer[0] - signalLineBuffer[0]) > 0;
...
void OpenBuyPosition()
{
    ...
    else
    {
        ...
        // 如果是常见可恢复错误，可以添加重试逻辑
        if(error == 135 || error == 136 || error == 137) // 价格变化相关的错误
        {
            // 可以添加重试逻辑
        }
    }
...
void ManagePosition()
{
    if(!UseTrailingStop || !hasPosition) return;
    ...
    for(int i = PositionsTotal() - 1; i >= 0; i--)
    {
           ...
           positionInfo.Magic() == MagicNumber)
        {
            ...
            {
                newSL = bid - TrailingStop_Points * _Point;
                
                // 确保新止损优于原止损且不劣于开仓价过多
                if(newSL > currentSL && 
                   newSL > positionInfo.PriceOpen() - MaxStopLoss_Points * _Point &&
                   (newSL - currentSL) >= TrailingStep_Points * _Point)
                {
                    if(trade.PositionModify(positionInfo.Ticket(), newSL, positionInfo.TakeProfit()))
                    {
                        Print("移动止损更新 - 新止损: ", newSL);
                    }
                    else
                    {
                        uint error = GetLastError();
                        Print("移动止损失败 - 错误代码: ", error, ", 描述: ", ErrorDescription(error));
                    }
                }
            }
            break;
